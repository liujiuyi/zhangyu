...
if ($_REQUEST['act']=='single_reply')
{
	...
	/* 获得图片 */
	$sql = $sql = "SELECT thumb_url, img_url FROM " .$ecs->table('goods_gallery'). " WHERE single_id = '$_REQUEST[id]'";
	...
	foreach ($single_img as $key => $gallery_img)
	{
		...
		$img_list[$key]['img_url'] = '../' . (empty($gallery_img['img_url']) ? $gallery_img['thumb_url'] : $gallery_img['img_url']);
	}
	/* 获取管理员的用户名和Email地址 */
...
//    check_authz_json('goods_manage');
    ...
    $single_id       = intval($_POST['id']);
    $is_show_index         = intval($_POST['val']);
	
	if($single_id > 0)
	{
		$sql = "UPDATE " . $GLOBALS['ecs']->table('single') . " SET is_show_index='$is_show_index' WHERE single_id='$single_id'";
		if($GLOBALS['db']->query($sql))
		{
			clear_cache_files();
			make_json_result($is_show_index);
		}
	}
}
...
function get_single_list()
{
	...
	$sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('order_info'). " WHERE order_status = 5 AND is_single IN (1, 2) $where";
	$filter['record_count'] = $GLOBALS['db']->getOne($sql);
	...
	/* 获取晒单列表 */
	$arr = array();
	$sql  = "SELECT * FROM " .$GLOBALS['ecs']->table('single'). " WHERE 1=1 $where " .
	" ORDER BY $filter[sort_by] $filter[sort_order] ".
	" LIMIT ". $filter['start'] .", $filter[page_size]";
	$res  = $GLOBALS['db']->query($sql);
