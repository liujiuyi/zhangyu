...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if($rank <> $check_user_rank && $rank == 2) {
      // 发送红包
      $day    = getdate();
      $today  = local_mktime(23, 59, 59, $day['mon'], $day['mday'], $day['year']);
      $bonus_sql = "SELECT type_id FROM " . $ecs->table('bonus_type') . " WHERE send_start_date <= '$today' AND send_end_date >= '$today'";
...
{
   $sql = "SELECT u.user_id, u.user_name, a.user_name as parent_admin_name, s.shop_name, s.shop_contacts, s.shop_mobile, s.shop_address, u.reg_time FROM " . $ecs->table('users') . " u left join ".  $ecs->table('user_shop') . " s on u.user_id = s.user_id left join " . $ecs->table('admin_user') . " a on u.parent_admin_id = a.user_id";
   $users = $db->getAll($sql);
   $str = "会员ID,会员名称,销售员,店铺名称,店铺联系人,店铺电话,店铺地址,注册日期\n";
   ...
   foreach ($users as $key => $val)
   {
      ...
      $str .= $user_id.",".$user_name.",".$parent_admin_name.",".$shop_name.",".$shop_contacts.",".$shop_mobile.",".$shop_address.",".$reg_time."\n"; //用引文逗号分开
   }
   //设置文件名
