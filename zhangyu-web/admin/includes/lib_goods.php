 ...
 * @return  void
 */
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls, $single_id = 0)
{
    /* 是否处理缩略图 */
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original, single_id) " .
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT goods_id, is_hot_comment, goods_name, goods_type, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, is_like, sort_order, goods_number, integral, " .
