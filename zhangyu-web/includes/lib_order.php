...
function order_goods($order_id)
{
    $sql = "SELECT o.rec_id, o.goods_id, o.goods_name, o.goods_sn, o.market_price, o.goods_number, " .
            ...
            "o.goods_price * o.goods_number AS subtotal, o.extension_code ,g.thumb_url " .
            "FROM " . $GLOBALS['ecs']->table('order_goods') ." AS o ".
			" LEFT JOIN ".$GLOBALS['ecs']->table('goods_gallery')." AS g ON o.goods_id=g.goods_id".
            " WHERE order_id = '$order_id' GROUP BY o.rec_id";
 ...
 * 
 */
function recommend_integral($order_id)
{
   if ($order_id > 0)
   {
      $sql = "SELECT o.*, u.parent_id FROM " . $GLOBALS['ecs']->table('order_info') . " o," . $GLOBALS['ecs']->table('users') . " u 
      WHERE o.order_id = '$order_id' and o.user_id = u.user_id";
      $order = $GLOBALS['db']->getRow($sql);
      if ($order)
      {
        $goods_amount = $order['goods_amount'];
        $parent_id = $order['parent_id'];
        if($parent_id > 0 && $goods_amount > 0)
        {
          $sql = "UPDATE " . $GLOBALS['ecs']->table('users') . " SET pay_points = pay_points + ('$goods_amount') WHERE user_id = '$parent_id' LIMIT 1";
          $GLOBALS['db']->query($sql);
        }
      }
   }
