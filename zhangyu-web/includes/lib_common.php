...
function get_articles_cat_name($cat_name,$limit)
{
	...
	$sql = 'SELECT a.article_id, a.title, ac.cat_name, a.add_time, a.file_url, a.open_type, ac.cat_id, ac.cat_name ' .
            ...
            ' WHERE a.is_open = 1 AND a.cat_id = ac.cat_id AND ac.cat_type = 1 AND ac.cat_name="' .$cat_name.
...
function get_brands_index($cat = 0, $app = 'brand')
{
    ...
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.brand_desc, IF(b.brand_logo > '', '1', '0') AS tag ".
            "FROM " . $GLOBALS['ecs']->table('brand') .
            " AS b ORDER BY tag DESC, b.sort_order ASC";
    if (isset($static_page_libs[$template]['/library/brands.lbi']))
    {
        $num = get_library_number("brands");
        $sql .= " LIMIT $num ";
...
/**
 * 获得指定分类下所有底层分类的ID修改版本
 *
 * @access  public
 * @param   integer     $cat        指定的分类ID
 * @return  string
 */
function get_children_pro($cat = 0)
{
 ...
 *
 * @access  public
 * @param   integer $ad_id     广告位ID
 * @return  string

 *author guan
 */
function get_hot_comment_ads($ad_id)
{
    static $static_res = NULL;

    $time = gmtime();

	$sql  = 'SELECT goods_id, goods_thumb ' .
			'FROM ' . $GLOBALS['ecs']->table('goods') . " WHERE is_hot_comment='1' limit 4";
	$res = $GLOBALS['db']->GetAll($sql);
	...
	foreach($res as $key => $val)
	{
		...
		{
			$sql  = 'SELECT content ' .
			'FROM ' . $GLOBALS['ecs']->table('comment') . " WHERE comment_rank >= '4' AND status = '1' AND id_value='$val[goods_id]'";
			$comment = $GLOBALS['db']->getRow($sql);
			$res[$key]['hot_comments'] = $comment['content'];
		}
		$res[$key]['ad_code'] = $val['goods_thumb'];
		$res[$key]['ad_link'] = "goods.php?id=$val[goods_id]";
	}
 ...
 * @return  array
 */
function get_single_show()
{
	/* 获取晒单列表 */
	$arr = array();
	$sql  = "SELECT single_id, order_id, single_name FROM " .$GLOBALS['ecs']->table('single'). " WHERE is_show_index='1'" .
	" ORDER BY addtime ";
	$res  = $GLOBALS['db']->getAll($sql);
	...
	{
		$sql = "SELECT thumb_url FROM " . $GLOBALS['ecs']->table('goods_gallery') . "WHERE single_id=" . $row['single_id'];
		$goods_gallery = $GLOBALS['db']->getRow($sql);

		
		$sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('order_goods') . "WHERE order_id=" . $row['order_id'];
		$goods = $GLOBALS['db']->getRow($sql);
		
		$rows[$k]['single_name'] = $row['single_name'];
		$rows[$k]['thumb_url'] = $goods_gallery['thumb_url'];
		$rows[$k]['single_id'] = $row['single_id'];
		$rows[$k]['goods_id'] = $goods['goods_id'];
		$k++;
	}
