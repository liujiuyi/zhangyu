...
class GoodsBaseModel extends BaseModel {
    ...
    function get_final_price2($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array()) {
        $final_price = '0'; //商品最终购买价格
        $volume_price = '0'; //商品优惠价格
        $promote_price = '0'; //商品促销价格
        $user_price = '0'; //商品会员价格
        ...
        $sql = "SELECT g.promote_price, g.promote_start_date, g.promote_end_date, " .
                "g.shop_price, " .
                "IFNULL(mp.user_price, g.shop_price * '" . $_SESSION['discount'] . "') AS rank_shop_price " .
                " FROM " . $this->pre . "goods AS g " .
                " LEFT JOIN " . $this->pre . "member_price AS mp " .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '" . $_SESSION['user_rank'] . "' " .
                " WHERE g.goods_id = '" . $goods_id . "'" .
        ...
        //取得商品会员价格列表
        $user_price = $goods['shop_price'];
        $rank_user_price = $goods['rank_shop_price'];
        ...
        if ($is_spec_price) {
            if (!empty($spec)) {
                $spec_price = model('Goods')->spec_price($spec);
                $final_price += $spec_price;
                $rank_final_price = $rank_user_price + $spec_price;
            }
        }

        $all_price = array();
        $all_price['final_price'] = $final_price;
        $all_price['rank_final_price'] = $rank_final_price;
        //返回商品最终购买价格
        return $all_price;
    }
