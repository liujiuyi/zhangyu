...
class UserController extends CommonController {
    ...
    public function index() {
        ...
        $sql = "SELECT is_invite FROM "  . $this->model->pre . "users WHERE is_invite = 1 and user_id = '".$this->user_id."'";
        $rs = $this->model->query($sql);
        if ($rs) {
           $sql = "SELECT LPAD(user_id, 4, 0) invite_code FROM "  . $this->model->pre . "users WHERE user_id = '".$this->user_id."'";
           $invite_info = $this->model->getRow($sql);
           $this->assign('invite_code', 'UU'.$invite_info['invite_code']);
        }
    ...
    public function user_shop() {
       if (IS_POST) {
        $user_shop = array(
          ...
          'shop_contacts' => I('post.shop_contacts'),
          'shop_mobile' => I('post.shop_mobile'),
          'shop_address' => I('post.shop_address')
        );
       
        if (model('Users')->update_user_shop($user_shop)) {
         //检查邀请码
         $shop_invite_code = I('post.shop_invite_code');
         if($shop_invite_code && strlen($shop_invite_code) == 6){
          ...
          if(strncasecmp($invite_mark, "au", 2) == 0){
           $invite_sql = 'update ' . $this->model->pre . 'users set parent_admin_id = ' . $invite_id . " WHERE user_id=" . $this->user_id . " and parent_admin_id <= 0";
          }else if(strncasecmp($invite_mark, "uu", 2) == 0){
           $invite_sql = 'update ' . $this->model->pre . 'users set parent_id = ' . $invite_id . " WHERE user_id=" . $this->user_id . " and parent_id <= 0";
          }
          $this->model->query($invite_sql);
         }
         show_message('店铺注册成功，请等待审核', '返回用户中心', url('index'));
        }
        exit();
       }
       // 获得用户对应店铺信息
       $user_shop = model('Users')->get_user_shop_info($_SESSION['user_id']);
       $this->assign('user_shop', $user_shop);
       // 获得用户对应的图片信息
       $user_shop_pic = model('Users')->get_user_shop_pic_info($_SESSION['user_id']);
       $this->assign('user_shop_pic', $user_shop_pic);
       
       $this->display('user_shop.dwt');
    }

    /**
     * 店铺图片的处理
     */
    public function user_shop_upload() {
       $typeArr = array("jpg", "png", "gif", "jpeg");//允许上传文件格式
       $path = "data/attached/user_shop_image/";//上传路径
       if (IS_POST) {
         ...
         if (move_uploaded_file($name_tmp, $pic_url)) { //临时文件转移到目标文件夹
           ...
           model('Users')->update_user_shop_pic($user_shop_pic);
     ...
     * 店铺图片重置处理
     */
    public function user_shop_reset() {
      model('Users')->drop_user_shop_pic($this->user_id);
      echo 'true';
    }
}
