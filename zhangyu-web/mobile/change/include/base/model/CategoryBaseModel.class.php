...
class CategoryBaseModel extends BaseModel {
    ...
    function get_categories_tree($cat_id = 0) {
        ...
        if ($this->row($sql) || $parent_id == 0) {
            /* 获取当前分类及其子分类 */
            $sql = 'SELECT c.cat_id,c.cat_name,c.parent_id,c.is_show,t.cat_image,c.cat_ads,c.cat_ads_url ' .
            ...
            foreach ($res AS $row) {
                if ($row['is_show']) {
                    ...
                    $cat_arr[$row['cat_id']]['cat_ads'] = $row['cat_ads'];
                    $cat_arr[$row['cat_id']]['cat_ads_url'] = $row['cat_ads_url'];
