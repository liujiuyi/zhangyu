...
class CategoryController extends AdminController {
    ...
    public function edit() {
        if (IS_POST) {
            ...
            /* 更新栏目图标 */
            if ($_FILES['cat_ads']['name']) {
                /* cat_image图标 */
                $result = $this->ectouchUpload('cat_ads', 'cat_image');
                if ($result['error'] > 0) {
                    $this->message($result['message'], NULL, 'error');
                }
                $cat_info['cat_ads'] = substr($result['message']['cat_ads']['savepath'], 2) . $result['message']['cat_ads']['savename'];
            }
            /* 更新栏目 */
