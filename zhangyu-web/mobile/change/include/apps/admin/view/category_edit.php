    ...
    <form action="{url('edit')}" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
      <table id="general-table" class="table table-hover ectouch-table">
        ...
        <tr>
          <td>{$lang['cat_ads']}:</td>
          <td><div class="col-md-4">
              <input type="file" name="cat_ads" class="form-control input-sm" />
              </div>
              {if $cat_info['cat_ads']}
              <div class="col-md-1">
                <a href="javascript:;" class="glyphicon glyphicon-picture ectouch-fs16" style="text-decoration:none;" onClick="showImg('cat_ads_layer', '{$lang['cat_ads']}')" title="View"></a>
				<div id="cat_ads_layer" style="display:none"> <img src="{$cat_info['cat_ads']}" border="0" style="max-width:320px; max-height:320px;" /> </div>
              </div>
              {/if}
          </td>
        </tr>
        <tr>
          <td>{$lang['cat_ads_url']}:</td>
          <td><div class="col-md-4">
              <input type="text" name="data[cat_ads_url]" value="{$cat_info['cat_ads_url']}" class="form-control input-sm" />
            </div></td>
        </tr>
        <tr>
          <td>{$lang['grade']}:</td>
