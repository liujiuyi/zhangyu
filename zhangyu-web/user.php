...
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer', 'user_shop');
 $smarty->assign('categories',       get_categories_tree_pro()); // 分类树
/* 未登录处理 */
...
/* 批发商申请页面 */
elseif ($action == 'user_shop')
{
    $sql = 'SELECT * FROM ' . $ecs->table('user_shop') . ' WHERE user_id = ' . $_SESSION['user_id'];
    $user_shop = $db->getRow($sql);
    $smarty->assign('user_shop', $user_shop);
    // 获得用户对应的图片信息
    $user_shop_pic = array();
    $sql = 'SELECT * FROM ' . $ecs->table('user_shop_pic') . ' WHERE user_id = ' . $_SESSION['user_id'];
    $res = $db->query($sql);
    while ($item = $db->fetch_array($res))
    {
        $user_shop_pic[]  = $item;
...
/* 修改批发商资料的处理 */
elseif ($action == 'act_user_shop')
{
    $shop_id = isset($_POST['shop_id']) ? trim($_POST['shop_id']) : '';
    $shop_name = isset($_POST['shop_name']) ? trim($_POST['shop_name']) : '';
    $shop_contacts = isset($_POST['shop_contacts']) ? trim($_POST['shop_contacts']) : '';
    $shop_mobile = isset($_POST['shop_mobile']) ? trim($_POST['shop_mobile']) : '';
    $shop_address = isset($_POST['shop_address']) ? trim($_POST['shop_address']) : '';

    $shop_invite_code = isset($_POST['shop_invite_code']) ? trim($_POST['shop_invite_code']) : '';
    
    $user_id = $_SESSION['user_id'];
    if (!empty($user_id)){
       $shop_id = intval($shop_id);
       
       if ($shop_id > 0)//如果之前没有申请过，则插入
       {
           $sql = 'UPDATE ' . $ecs->table('user_shop') . " SET shop_name = '$shop_name', shop_contacts = '$shop_contacts', shop_mobile = '$shop_mobile', shop_address = '$shop_address' WHERE shop_id = '$shop_id'";
       }
       else
       {
           $sql = 'SELECT * FROM ' . $ecs->table('user_shop') . ' WHERE user_id = ' . $user_id;
           $check_user_shop = $db->getRow($sql);
           if($check_user_shop) {
             $shop_id = $check_user_shop['shop_id'];
             $sql = 'UPDATE ' . $ecs->table('user_shop') . " SET shop_name = '$shop_name', shop_contacts = '$shop_contacts', shop_mobile = '$shop_mobile', shop_address = '$shop_address' WHERE shop_id = '$shop_id'";
           } else {
             $sql = 'INSERT INTO '. $ecs->table('user_shop') . " (`user_id`, `shop_name`, `shop_contacts`, `shop_mobile`, `shop_address`) VALUES ('$user_id', '$shop_name', '$shop_contacts', '$shop_mobile', '$shop_address')";
           }
       }
       $db->query($sql);
       if (!empty($shop_invite_code)){
          if($shop_invite_code && strlen($shop_invite_code) == 6){
            $invite_mark = substr($shop_invite_code, 0, 2);//au是销售员，uu是普通用户
            //取得邀请码的账号ID
            $pattern = "/^(0+)(\d+)/i";
            $replacement = "\$2";
            $invite_id = preg_replace($pattern, $replacement, substr($shop_invite_code,2));
       ...
       show_message('店铺注册成功，请等待审核', '查看我的店铺信息', 'user.php?act=user_shop', 'info');
    } else {
       show_message('店铺注册失败，请稍后重试', '查看我的店铺信息', 'user.php?act=user_shop', 'info');
    }
}

/* 批发商店铺照片的处理 */
elseif ($action == 'user_shop_upload')
{
   $typeArr = array("jpg", "png", "gif", "jpeg");//允许上传文件格式
   $path = "mobile/data/attached/user_shop_image/";//上传路径
   ...
   $data_pic_url = 'data/attached/user_shop_image/' . $pic_name;//上传后图片路径+名称
   if (move_uploaded_file($name_tmp, $pic_url)) { //临时文件转移到目标文件夹
     $user_id = $_SESSION['user_id'];
     $sql = 'INSERT INTO '. $ecs->table('user_shop_pic') . " (`user_id`, `pic`) VALUES ('$user_id', '$data_pic_url')";
     $db->query($sql);
...
/* 批发商店铺照片的重置处理 */
elseif ($action == 'drop_user_shop_pic')
{
     $user_id = $_SESSION['user_id'];
     $sql = 'DELETE FROM '. $ecs->table('user_shop_pic') . " WHERE user_id = $user_id";
     $db->query($sql);
     echo 'true';
}
?>
